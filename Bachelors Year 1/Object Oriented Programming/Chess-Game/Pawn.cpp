...
bool Pawn::valid ( std::pair < int, int > mPair, Pieces* const ChessPiece[8][8]) const {
...
std::vector<std::pair<int, int>> Pawn::getPossibleMoves(Pieces* const ChessPiece[8][8]) const{

    std::vector < std::pair < int, int > > mList;
    std::pair < int, int > aux;
    if( getColor() == Pieces::WHITE )
    {
        if( getMoved() )
        {
            aux = std::make_pair ( m_pozition.first + 1, m_pozition.second );
            if( valid ( aux, ChessPiece ) )
               if( ChessPiece[m_pozition.first + 1][m_pozition.second] == NULL )
                    mList.push_back( aux );
        }
        else
        {
            ...
            if( valid ( aux, ChessPiece ) )
                ...
                {
                    mList.push_back( aux );
                    aux = std::make_pair ( m_pozition.first + 2, m_pozition.second );
                    if( valid ( aux, ChessPiece ) )
                       if( ChessPiece[m_pozition.first + 2][m_pozition.second] == NULL )
                            mList.push_back( aux );
                }
        }
        aux = std::make_pair ( m_pozition.first + 1, m_pozition.second - 1 );
        if( valid ( aux, ChessPiece ))
            if( ChessPiece[m_pozition.first + 1][m_pozition.second - 1] )
                mList.push_back( aux );
        aux = std::make_pair ( m_pozition.first + 1, m_pozition.second + 1 );
        if( valid ( aux, ChessPiece ))
            if( ChessPiece[m_pozition.first + 1][m_pozition.second + 1] )
